...
// ##################################################################
// Custom ReviewTest class.
//
//
// inherit from ListeningTest
function ReviewTest(TestData) {
    ListeningTest.apply(this, arguments);
}
ReviewTest.prototype = new ListeningTest();
ReviewTest.prototype.constructor = ReviewTest;
...
// ###################################################################
// create random mapping to test files
ReviewTest.prototype.createFileMapping = function (TestIdx) {
    var NumFiles = $.map(this.TestConfig.Testsets[TestIdx].Files, function(n, i) { return i; }).length;
    var fileMapping = new Array(NumFiles);    
...
// ###################################################################
// read ratings from TestState object
ReviewTest.prototype.readRatings = function (TestIdx) {
...
// ###################################################################
// save ratings to TestState object
ReviewTest.prototype.saveRatings = function (TestIdx) {
    var ratings = new Object();
    $(".rateSlider").each( function() {
        var pos = $(this).attr('id').lastIndexOf('slider');
        var fileNum = $(this).attr('id').substring(pos+6, $(this).attr('id').length);
...
ReviewTest.prototype.createTestDOM = function (TestIdx) {
        ...
        var reviewConf = this.TestConfig;
        $('.rateSlider').each( function() {
            $(this).slider({
                    value: reviewConf.RateDefaultValue,
                    min: reviewConf.RateMinValue,
                    max: reviewConf.RateMaxValue,
                    animate: false,
                    orientation: "horizontal"
            });
                    
            $(this).slider('option', 'value', 0);
            $(this).css('background-image', 'url('+reviewConf.RateScaleBgPng+')');
        });

}

ReviewTest.prototype.formatResults = function () {
